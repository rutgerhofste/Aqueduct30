...
assetNameICs = ee.data.getList({"id":'users/rutgerhofste/meanNonAccumulatedPCRRunoff'})


assetNameImages =[]
for assetNameIC in assetNameICs:
    assetNameImages.append(ee.data.getList({"id":assetNameIC["id"]}))

assetNameImages = list(itertools.chain.from_iterable(assetNameImages))


exportDescriptions = ee.List(map(getExportDescription,assetNameImages)).getInfo() # Client side!
clientDict = dict(zip(exportDescriptions,assetNameImages))
...
for key, value in clientDict.iteritems():
    ...
    ##image = volumeToFlux(image) # IMAGES ALREADY IN M!!!
